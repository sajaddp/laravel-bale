...
it('asks a user to review the bot with Bale documented integer parameters', function (): void {
    Http::fake([
        'https://tapi.bale.ai/bottest-token/askReview' => Http::response(['ok' => true, 'result' => true]),
    ]);

    expect(Bale::askReview(userId: 123456789, delaySeconds: 30))->toBeTrue();

    Http::assertSent(function (Request $request): bool {
        return $request->url() === 'https://tapi.bale.ai/bottest-token/askReview'
            && $request->method() === 'POST'
            && $request->data() === [
                'user_id' => 123456789,
                'delay_seconds' => 30,
...
it('keeps Bale API error parsing for askReview', function (): void {
    Http::fake([
        'https://tapi.bale.ai/bottest-token/askReview' => Http::response([
            'ok' => false,
            'error_code' => 400,
            'description' => 'User is unavailable.',
        ], 400),
    ]);

    expect(fn (): bool => Bale::askReview(userId: 123456789, delaySeconds: 0))
        ->toThrow(BaleRequestException::class);
});

it('requires Bale documented boolean results for askReview', function (): void {
    Http::fake([
        'https://tapi.bale.ai/bottest-token/askReview' => Http::response(['ok' => true, 'result' => []]),
    ]);
